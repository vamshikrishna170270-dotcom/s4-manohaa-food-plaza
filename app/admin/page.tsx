...
function MenuView({ dishes, categories, setDishes, uploadImage }: any) {
  ...
  const handleDelete = async (id: string) => {
    ...
    // Optimistic UI Update (Removes it instantly from the screen)
    ...
    if (error) {
      ...
      setDishes(previousDishes); // Revert if it fails
    }
  };
              ...
              <motion.div key={d.id} layout initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.94 }} className="relative group">
                <Glass className="overflow-hidden border-white/5 h-full flex flex-col">
                  <div className="relative h-40 overflow-hidden shrink-0">
                    ...
                    {/* Veg/Non-Veg Icon */}
                    <div className="absolute bottom-3 right-3 bg-black/60 backdrop-blur-md px-1.5 py-1.5 rounded-md shadow-sm border border-white/10 z-10">
                    ...
                    <div className="absolute top-3 right-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                      <button onClick={() => handleEdit(d)} className="p-2 bg-black/60 backdrop-blur-md rounded-md border border-white/20 text-white hover:bg-[#D4AF37] hover:text-black hover:border-[#D4AF37]" title="Edit Dish">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(d.id)} className="p-2 bg-black/60 backdrop-blur-md rounded-md border border-white/20 text-white hover:bg-red-500 hover:text-white hover:border-red-500" title="Delete Dish">
...
function AddDishModal({ open, onClose, dishToEdit, onSave, categories, uploadImage }: any) {
  ...
  // Mounted state ensures createPortal works smoothly with Next.js SSR
  ...
  useEffect(() => {
    if (open) {
      if (dishToEdit) {
        ...
        setPopular(false); setIsVeg(true); setPreview(null); setImageFile(null);
      }
    }
  ...
  const submit = async () => {
    ...
    try {
      ...
      const payload = { name, desc, price: Number(price), category_id: cat, img: finalImgUrl, popular, is_veg: isVeg };
      let data, error;
  ...
  // 🔴 MAGIC FIX: createPortal pulls the modal out of the scrolling container and locks it to your screen
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 bg-gradient-to-t from-[#09090B] via-[#09090B]/95 to-transparent z-20">
          <div className="max-w-3xl mx-auto relative flex items-center bg-[#1a1a1d] rounded-full border border-white/15 p-2 shadow-2xl focus-within:border-[#D4AF37] transition-colors">
            ...
            <input 
              ...
              placeholder={isListening ? "Listening... Speak now..." : "Ask Sommelier to analyze your restaurant ledger..."} 
              className="w-full bg-transparent border-none outline-none text-white pl-12 pr-28 py-3 text-sm sm:text-base font-medium placeholder:text-gray-500" 
            />
            
            <button 
              onClick={toggleSpeech}
              className={`absolute right-14 w-10 h-10 rounded-full flex items-center justify-center transition-all ${isListening ? 'bg-red-500/20 text-red-500 animate-pulse' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
