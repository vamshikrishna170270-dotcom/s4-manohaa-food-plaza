...
export default function CommandCenter() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (password === 'manohaa123') { sessionStorage.setItem('adminAuth', 'true'); setIsAuthenticated(true); } 
...
function Sidebar({ view, setView }: { view: ViewId; setView: (v: ViewId) => void }) {
  return (
    <aside className="sticky top-0 hidden h-screen w-[248px] shrink-0 flex-col border-r border-white/[0.07] bg-[#121214]/70 px-5 py-8 backdrop-blur-xl md:flex">
      <div className="mb-10 flex items-center gap-3 px-2"><span className="grid h-10 w-10 place-items-center rounded-2xl border border-[#D4AF37]/30 bg-[#D4AF37]/10"><Crown className="h-5 w-5 text-[#D4AF37]" /></span><div className="leading-tight"><p className="font-serif text-[1.05rem] tracking-wide">S4 Manohaa</p><p className="text-[0.62rem] uppercase tracking-[0.3em] text-[#D4AF37]/70">Food Plaza</p></div></div>
